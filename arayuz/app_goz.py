...
st.set_page_config(
    page_title="Göz Analiz Arayüzü",
    page_icon="🩺",
    layout="wide",
)

BASE_DIR = Path(__file__).resolve().parents[1]
MODEL_PATH = BASE_DIR / "modeller" / "goz_final_model.keras"
IMG_SIZE = (224, 224)
CLASS_LABELS = ["CNV", "DME", "DRUSEN", "NORMAL"]

CLASS_INFO = {
    "CNV": "Choroidal Neovascularization; retinada anormal damar oluşumu ile ilişkilidir.",
    "DME": "Diyabetik Makula Ödemi; makülada sıvı birikimi ile görme kaybına neden olabilir.",
    "DRUSEN": "Retinal pigment epiteli altında birikimler; makula dejenerasyonu ile ilişkili olabilir.",
    "NORMAL": "Retina ve makula yapıları normal görünmektedir.",
}

RESULT_CLASSES = {
    "CNV": "result-cnv",
    "DME": "result-dme",
    "DRUSEN": "result-drusen",
...
st.markdown(
    ...
    .subtitle { color: #334155; margin-top: 0.45rem; margin-bottom: 0; }
    .result-box {
        border-radius: 16px;
        padding: 1rem 1.1rem;
    ...
    .result-normal { background: #e9f8ef; color: #166534; border-color: #86efac; }
    .result-cnv { background: #fef1f2; color: #9f1239; border-color: #fda4af; }
    .result-dme { background: #fff7ed; color: #9a3412; border-color: #fdba74; }
    .result-drusen { background: #fef3c7; color: #92400e; border-color: #fde68a; }
    .info-card {
        background: #ffffff;
        border: 1px solid var(--border);
        border-radius: 14px;
...
st.markdown(
    """
    <div class="app-shell">
        <h1 class="main-title">Yapay Zeka Destekli Göz Analiz Sistemi</h1>
        <p class="subtitle">Retina görüntülerini eğitilmiş model ile analiz eder.</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
with left_col:
    uploaded_file = st.file_uploader(
        "PNG, JPG veya JPEG formatında göz retinal görüntüsü seçin.",
        type=["png", "jpg", "jpeg"],
    )
